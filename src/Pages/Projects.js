        ...
        <div className="flex w-10/12 md:w-full mx-auto flex-col md:flex-row flex-wrap justify-center items-center gap-10">
          {ProjectData.map((project) => {
            return (
              <div
                key={project.id}
                className="flex bg-slate-100 rounded-md md:w-3/12 md:min-h-[400px] flex-col hover:shadow-lg hover:cursor-pointer transition-all duration-300"
              >
                <img
                  src={project.image}
                  alt={project.name}
                  className="max-h-[200px] w-screen rounded-lg p-2"
                />
                <div className="flex flex-col text-center justify-center md:w-10/12 md:mx-auto">
                  ...
                  <p className="font-lora md:p-2 p-3 font-normal text-gray-500 md:text-1xl text-left">
                    <span className="font-bold">Skills </span> : {project.skills}
                  </p>
                </div>
              </div>
            );
